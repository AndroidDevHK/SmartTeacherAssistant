...
public class ViewStudentQuizResultsActivity extends AppCompatActivity {

    private static final String TAG = "AvailableQuizzesActivity";
    private RecyclerView recyclerViewAvailableQuizzes;
    private QuizResultAdapter quizAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_view_student_quiz_results);
        ...
        quizAdapter = new QuizResultAdapter(availableQuizzesList);
    ...
    private void SetupToolbar(Toolbar toolbar) {
        SetupToolbar.setup(this, toolbar, "View Quiz Results", true);
    }
    ...
    private void showNoQuizzesDialog() {
        ...
        new AlertDialog.Builder(this)
                .setTitle("No Quiz Results Available")
                .setMessage("There are currently no quiz results available for you.")
                .setPositiveButton("OK", (dialog, which) -> {
                    dialog.dismiss();
                    finish(); // Finish the activity after dismissing the dialog
                })
                .show();
    }
    ...
    private List<StudentQuizModel> filterQuizzesByTime(List<StudentQuizModel> allQuizzes, String currentDateTime) {
        List<StudentQuizModel> pastQuizzes = new ArrayList<>();
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss'Z'", Locale.getDefault());
        sdf.setTimeZone(TimeZone.getTimeZone("Asia/Karachi"));
        Date currentDate;
        try {
        ...
        for (StudentQuizModel quiz : allQuizzes) {
            try {
                ...
                // Include quizzes that have ended
                if (quizEndDate.before(currentDate)) {
                    pastQuizzes.add(quiz);
                    Log.d(TAG, "filterQuizzesByTime: Quiz ID: " + quiz.getQuizId() + " has ended and is included");
                } else {
                    Log.d(TAG, "filterQuizzesByTime: Quiz ID: " + quiz.getQuizId() + " has not ended yet and is not included");
                }
            } catch (ParseException e) {
                e.printStackTrace();
